...
export function EmailComposerModal({ isOpen, onClose, mode = 'individual', preselectedClientId, preselectedContactId }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Compor e-mail" maxWidth={showPreview ? 'max-w-5xl' : 'max-w-2xl'}>
      ...
      {result ? (
        ...
        <div className={showPreview ? 'flex gap-6' : 'space-y-4'}>

          <div className={showPreview ? 'flex-1 space-y-4 min-w-0' : 'space-y-4'}>

            {/* Empresa */}
            <div>
              <label className="block text-xs font-semibold text-text-tertiary uppercase tracking-wide mb-1">Empresa</label>
              {client ? (
                ...
                <div className="relative">
                  <input type="text" placeholder="Buscar empresa..." value={clientSearch}
            ...
            {client && (
              <div>
                <label className="block text-xs font-semibold text-text-tertiary uppercase tracking-wide mb-1">Para</label>
                {loadingContacts ? (
                      ...
                      {selectedIds.map(id => {
                        ...
                        return (
                          <span key={id}
                            ...
                            <button onClick={() => toggleContact(id)} className="hover:text-red-500 shrink-0">
                    ...
                    {contactSearch.trim() && filteredContacts.length > 0 && (
                      <div className="absolute z-10 w-full mt-1 bg-bg-primary border border-border-tertiary rounded-md shadow-lg max-h-48 overflow-y-auto">
                        {filteredContacts.map(link => {
                          ...
                          const email = getContactEmail(link)
                          return (
                            <button key={link.contact_id}
                              onClick={() => selectContact(link.contact_id)} disabled={!email}
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="fromMode" value="csm"
                      checked={fromMode === 'csm'} onChange={() => setFromMode('csm')}
                      disabled={!profile?.email?.endsWith('@donc.com.br')} className="accent-donc-sky" />
                    <span className="text-sm text-text-primary">Meu e-mail ({profile?.email})</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="fromMode" value="noreply"
                      checked={fromMode === 'noreply'} onChange={() => setFromMode('noreply')}
          ...
          {showPreview && selectedTemplate && (
            <div className="w-80 shrink-0 space-y-4 border-l border-border-tertiary pl-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}>
              <div>
