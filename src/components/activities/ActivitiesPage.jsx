...
export default function ActivitiesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter.length) {
      ...
      list = list.filter(a => {
        if (statusFilter.includes('pendentes') && a.status === 'pendente' && (!a.due_date || a.due_date >= today)) return true
        if (statusFilter.includes('concluidas') && a.status === 'concluida') return true
        if (statusFilter.includes('atrasadas') && a.status === 'pendente' && a.due_date && a.due_date < today) return true
        return false
      })
    }
    if (clientFilter) list = list.filter(a => String(a.client_id) === clientFilter)
    if (respFilter) list = list.filter(a => a.responsible_id === respFilter)
    return list
  }, [activities, tab, search, statusFilter, clientFilter, respFilter])

  const pending = filtered.filter(a => a.status === 'pendente').sort((a,b) => a.activity_date?.localeCompare(b.activity_date))
  const done = filtered.filter(a => a.status === 'concluida')
  ...
  return (
    <div className="p-6">
      <PageHeader
        ...
        action={<Button onClick={() => setShowCreate(true)}>+ Nova Atividade</Button>}
      />
      ...
      <div className="relative max-w-sm mb-4">
        ...
        />
        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-text-tertiary pointer-events-none">🔍</span>
      </div>
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-border-secondary rounded-md hover:border-text-tertiary transition-colors bg-bg-primary"
          >
          ...
          {showStatusDD && (
            <>
              ...
              <div className="absolute top-full left-0 mt-1 w-40 bg-bg-primary border border-border-tertiary rounded-lg shadow-lg z-20 py-1">
                {['pendentes','concluidas','atrasadas'].map(s => (
                  <label key={s} className="flex items-center gap-2 px-3 py-1.5 hover:bg-bg-secondary cursor-pointer">
                    <input type="checkbox" checked={statusFilter.includes(s)} onChange={() => toggleStatus(s)} className="accent-donc-sky" />
        ...
        <div className="space-y-6">
          ...
          {pending.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {pending.map(a => <ActivityItem key={a.id} activity={a} onClick={() => setSelected(a)} />)}
              </div>
            </div>
          )}

          {/* Últimas (concluídas) por mês */}
          {doneGroups.map(([month, acts]) => (
            <div key={month}>
              ...
              <div className="space-y-2">
                {acts.map(a => <ActivityItem key={a.id} activity={a} onClick={() => setSelected(a)} />)}
              </div>
            </div>
          ))}

          {filtered.length === 0 && (
            <div className="text-center py-16 text-text-tertiary">Nenhuma atividade encontrada.</div>
          )}
        </div>
      )}

      {showCreate && <ActivityModal onClose={() => setShowCreate(false)} />}
    ...
    >
      ...
      <div className="flex flex-col items-end gap-1 flex-shrink-0">
        ...
        {a.activity_attachments?.length > 0 && (
          <span className="text-xs text-text-tertiary">📎 {a.activity_attachments.length}</span>
        )}
      </div>
    </div>
  )
