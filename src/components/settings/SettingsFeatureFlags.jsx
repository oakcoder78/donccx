...
const FLAG_GROUPS = {
  ...
  'Cockpits & Dashboards': ['health_cockpit', 'cs_radar', 'projects_cockpit', 'profissionais_cockpit', 'dashboard_v3', 'financial_data'],
}
...
export function SettingsFeatureFlags() {
  ...
  const groupFlags = (flags) => {
    const groups = {}
    for (const [groupKey, keys] of Object.entries(FLAG_GROUPS)) {
      groups[groupKey] = flags.filter(f => keys.includes(f.key))
    }
    const others = flags.filter(f => 
      !Object.values(FLAG_GROUPS).flat().includes(f.key)
    )
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {Object.entries(grouped).map(([groupName, groupFlags]) => (
        <div key={groupName}>
          <p className="text-xs font-semibold text-text-tertiary uppercase tracking-wider px-1 mb-3">
            {groupName}
          </p>
