...
import { ScopeLabel, scopeForRole } from './ScopeLabel'
import { Drawer } from '@/components/ui/Drawer'
import { OperationalHistoryDrawer } from './OperationalHistoryDrawer'

function Rows({ rows, kind, prevShort, prev2Short, onRow }) {
  const isHealth = kind === 'health'
  if (!rows.length) return <p style={{ margin: 0, color: C.ink2, fontSize: '0.8125rem' }}>Sem variação no período.</p>
  return (
      ...
      {rows.slice(0, 5).map(r => (
        <li key={r.clientId}>
          <button
            type="button"
            onClick={() => onRow(r)}
            style={{ width: '100%', textAlign: 'left', font: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '9px 0', borderBottom: `0.5px solid ${C.line}` }}
          >
            ...
            {isHealth
              ...
              : (r.state === 'new' ? <DeltaBadge neutralLabel="Início de uso" /> : <DeltaBadge pct={r.state === 'up' ? Math.abs(r.delta ?? 0) : -Math.abs(r.delta ?? 0)} />)}
          </button>
        </li>
      ))}
    </ul>
...
export function OperacionalVariacaoBlock({ deltas, effectiveRole, syncStatus }) {
  ...
  return (
    <Panel as="section" aria-labelledby="v3-op-title" style={{ gridColumn: '1 / -1' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, minHeight: 28 }}>
        ...
        <ScopeLabel scope={scopeForRole(effectiveRole)} />
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
            {panels.map(p => (
              <div key={p.kind}>
                ...
                <Rows rows={p.rows} kind={p.kind} prevShort={prevMonthShort} prev2Short={prevMonth2Short} onRow={r => openRow(p.mode, r)} />
                {p.rows.length > 5 && (
