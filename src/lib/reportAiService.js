...
function mountUserContent(sectionType, data, clientName, period) {
  ...
  switch (sectionType) {
    case 'escala':
      return `Analise os dados de escala operacional do cliente ${clientName} em ${periodLabel}:
...
Gere uma análise do desempenho operacional do período.`
    ...
    case 'qualidade_operacao':
      return `Analise a qualidade operacional do cliente ${clientName} em ${periodLabel}:
...
Gere uma análise focada em qualidade e oportunidades de melhoria.`
    ...
    case 'indicadores_operacionais':
      return `Analise os indicadores operacionais do cliente ${clientName} em ${periodLabel}:
...
Gere uma análise sobre eficiência operacional dos profissionais.`
    ...
    case 'categorias_ocorrencia':
      return `Analise as categorias de ocorrência do cliente ${clientName} em ${periodLabel}:
...
Gere uma análise identificando padrões e oportunidades de ação.`
    ...
    case 'desempenho_operacional':
      return `Analise o desempenho dos profissionais do cliente ${clientName} em ${periodLabel}:
...
Gere uma análise do desempenho geral da equipe de campo.`
    ...
    case 'suporte':
      return `Analise o suporte ao cliente ${clientName} em ${periodLabel}:
...
Gere uma análise da performance de suporte no período, destacando pontos de atenção.`
    ...
    default:
      return `Analise os dados operacionais do cliente ${clientName} em ${periodLabel}.`
  }
}

export async function generateSectionAnalysis({ sectionType, sectionData, clientName, period }) {
...
Não use markdown, apenas texto corrido.`
  ...
  const userContent = mountUserContent(sectionType, sectionData, clientName, period)
