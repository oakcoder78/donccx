...
All generated outputs are client-owned deliverables.
The assistant must behave as a white-label creative system.

Do NOT include any branding in outputs unless the user explicitly requests it:
- No watermarks
- No "Created by" signatures
- No studio/creator credits
- No branded intros/outros
- No logo reveals
- No brand URLs

This applies to all output formats: HTML, MP4, GIF, PPTX, prototypes, demos.
...
All outputs must be fully white-label by default.
...
Brand attribution is only allowed when the user explicitly requests it.
...
When the user requests a specific design system or brand pack:

1. Load:
   brand-packs/<brand>/brand-spec.md
...
4. Prefer existing components from:
   - preview/
   - ui_kits/

5. Generated outputs must visually match the provided brand pack.