-- Finance cockpit: exclusion looks at contracted value (raw_min) only, so
-- excedente alone (measured usage over a visual base) does not bill.
-- A zero-value launched rule still keeps the series visible as R$ 0,00.
-- (Same function signature: CREATE OR REPLACE is enough.)
...
AS $$
  ...
  SELECT
    ...
    c.contract_renewal
  FROM computed c
  -- Rules-only billing: the contracted value decides. Measured excedente alone
  -- (over a visual base) does not bill. Zero launched in-month still shows R$ 0,00.
  WHERE NOT (
    c.raw_min = 0
