-- Finance cockpit: rules-only billing + OS-created usage + eventuais + auto_renew default
-- 1) _financeiro_series_month: contratado = rules_total do mes (sem fallback base);
--    uso_os = SUM(snapshot.totalOs ?? os_created); exclui serie zerada sem lancamento
--    (pausa/zerada/isenta/regra-no-mes/excecao-na-serie permanecem visiveis).
-- 2) get_financeiro_cockpit: Uso/Billable por_os em OS criadas + coluna eventuais.
-- 3) get_financeiro_detalhe: payload eventuais do mes (linha propria).
-- 4) auto_renew = true onde billing_end IS NULL (29 mantem fim 2029).
...
AS $$
  ...
  base AS (
    SELECT
      ...
      (rm.series_id IS NOT NULL) AS has_rules_month,
      (exs.series_id IS NOT NULL) AS has_exc,
  ...
  WHERE NOT (
    ...
    AND NOT c.has_rules_month AND NOT c.has_exc
  );
$$;
...
BEGIN
  ...
  LEFT JOIN usage_os_cur uo ON uo.client_id = fc.client_id
  LEFT JOIN usage_os_prv uop ON uop.client_id = fc.client_id
  ...
  LEFT JOIN ev_cur ev ON ev.client_id = fc.client_id
...
DROP FUNCTION IF EXISTS public.get_financeiro_detalhe(int, text);
...
BEGIN
  ...
  SELECT
    ...
    (SELECT jsonb_agg(jsonb_build_object(
              ...
              'due_date', c.due_date
            ) ORDER BY c.ref_month, c.label)
     FROM public.contract_charges c
     WHERE c.client_id = p_client_id
       AND c.kind = 'implantacao'
       AND c.ref_month = p_ref_month),
    (SELECT jsonb_agg(jsonb_build_object(
              'nome', prof->>'nome',
              'email', prof->>'email',
...
UPDATE public.contract_series SET auto_renew = true WHERE billing_end IS NULL AND auto_renew = false;
