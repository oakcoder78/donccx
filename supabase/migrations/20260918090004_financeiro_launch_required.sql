-- Finance cockpit: launch-gated entries (repactuacao 2026-09-18)
-- A series appears iff it has launched values in the month (rules or tiers),
-- except paused / zerada / isenta which never appear. Excedente without launched
-- periods does not bill. (Same signature: CREATE OR REPLACE is enough.)
...
AS $$
  ...
  -- Launch-gated billing: a series bills iff it carries launched values in the
  -- month (recorrencia rules or OS tiers). Excedente only adds on top of launched
  -- rules. Paused / zerada (suspenso, nao_bilhetavel) / isencao_total never appear.
  WHERE (c.has_rules_month OR c.has_tiers)
