...
> 实战参考：[demos/hero-animation-v9.mp4](../demos/hero-animation-v9.mp4)
...
### 5. **Optional Corner Signature**

Gallery 阶段左上角可加轻量签名元素（如版本号、作品标识），16% opacity 色值，12px 字号，宽字距。

**为什么加这个**：
- Ripple 爆发后观众容易"失焦"，左上角轻标示帮助 anchor
- 在 GIF 被截屏分享时仍留下归属信号

**规则**：白标签时关闭，只在中段（画面 busy）出现，开场关闭（不遮 terminal），结尾关闭（内容是主角）。
