...
| **SettingsSyncStatus.jsx** | UI for monthly sync orchestration: run-now, set recurring schedule (presets: Mensal/Trimestral/Semestral + custom: semanas/meses/dias), schedule one-off execution, execution history table. BRT timezone (00:01 BRT). Triggered via Edge Function `sync-schedule`. |
