...
| `OnboardingDetailPage.jsx` | Detailed view of an onboarding project: header with "Questionários" button (`BriefHeaderButton`) opening `BriefPanelModal` with badge for unanswered client questions, timeline (fases), activities list with phase filtering, fase management panel with drag‑and‑drop. |
...
- **RLS (2026-09-13, two passes)** – `projects_admin_all` (admin/manager, ALL) was the only write policy; csm/sales only had SELECT scoped by the owning client's `csm_id`/`comercial_id`. First pass (`20260913200000_projects_owner_write.sql`) added `projects_csm_update`/`_delete` and `projects_sales_update`/`_delete`, still carteira-scoped, so owners could edit/delete their own client's projects. Second pass, same day (`20260913210000_relationship_tables_global_select.sql`), added `projects_global_select` (`USING (true)`) — SELECT is now global for every role, matching `clients_global_select`; only editing stays carteira-scoped.
