...
- **Modelo de acesso (2026-09-07, ver nota de 2026-09-13 abaixo):** `financial_data` (`20260824000006`) = `admin,manager,finance`; `SAFE_CLIENT_COLS` em `src/hooks/useClients.js:7` esconde `mrr/billing_*` de quem não tem a flag; `canSeeFinancial = admin/manager/finance`; leitura global de empresas (`20260903000001`) + sales escreve na carteira (`20260903000002`); só `admin/manager` acessam todas as tabs do detalhe.
  > **Nota (2026-09-13):** o mecanismo mudou, a decisão de negócio (Financeiro Cockpit restrito a admin/manager/finance) não. `SAFE_CLIENT_COLS` foi substituído pela view `public.clients_safe` (mascaramento no banco); `financial_data` ganhou `sales` no `allowed_roles` (hoje sem efeito prático aqui, já que o Cockpit em si continua exigindo só `cockpit_financeiro`); e as abas do detalhe do cliente (Atividades/Operacional/Health/Contatos) deixaram de ser restritas a admin/manager — ver `docs/modules/clients.md`.
...
- **Fase 1 aplicada (2026-09-11):** migration `20260911191431_financeiro_cockpit_core.sql` — `contract_series.usage_driven` (backfill 26/26 originals) + `correction_anniversary/percent/rule`; `billing_exceptions` (4 tipos; RLS select `admin,manager,finance,sales`, write `admin,manager,finance,sales` desde 2026-09-16; trigger `set_updated_at`); flag `cockpit_financeiro` (`false`, `admin,manager,finance`); RPCs `get_financeiro_cockpit`/`get_financeiro_detalhe`/`get_financeiro_export` + engine privado `_financeiro_series_month`. Smoke `2026-08`: 16 clientes, MRR real R$ 122.692,83, excedente R$ 16.587,36; guard `csm` → 42501; matemática conferida (Multiloja 260 × R$ 58,50 = R$ 15.210,00; Koerich 400 × R$ 40,00 = R$ 16.000,00).
...
- **Fase 3 implementada (2026-09-11):** `ExcecaoModal` (4 tipos, escopo cliente/série, sem retroativo, overlap warning, audit) + `PaymentToggle` (adimplência por série) + `useBillingExceptions`; ações na página; espelho no `ClientSubDados` e card read-only na aba Contrato (`ClientFormContent`) para admin/manager/finance/sales (Q8). Matriz RLS 2026-09-11 em produção (manager read-only, finance/admin write, sales lê e não escreve, cockpit bloqueia sales) — **revista 2026-09-16**: write em `billing_payments`/`billing_exceptions`/`contract_series` = `admin,manager,finance,sales` (migration `20260916120000`); leitura do cockpit (`cockpit_financeiro`) segue `admin,manager,finance`.
- **Fase 4 implementada (2026-09-11):** exports na página — visões geral/faturável/isento (isento removida 2026-09-18), CSV sintético (com escopo e escape), CSV analítico global (RPC `get_financeiro_export`) e por cliente (detalhe), PDF por cliente com CNPJ/SaaS_ID e rodapé de build; fix de data BRT.
...
- **MRR obrigatória + redesign do Contrato (2026-09-17/18, pré-produção):** fonte de verdade = **só o lançado** (períodos/faixas/eventuais); base do plano é referência visual e nunca fatura. `_financeiro_series_month`: contratado = `rules_total` do mês (sem fallback base); `uso_os` = OS criadas (`snapshot.totalOs ?? os_created`, soma instâncias); **só valor faturado aparece** — exclusão `raw_min==0 AND raw_real==0` sem isenções (não-cobrar/suspenso/isento/pausado nunca aparecem, mesmo com regra; regra 0 com excedente mostra o excedente). `get_financeiro_cockpit`: Uso/Billable `por_os` em OS criadas + coluna `eventuais` + clientes só-eventuais via `UNION`; `get_financeiro_detalhe`: payload `eventuais` (linha própria, soma no total do mês). Form: cabeçalho reagrupado (Assinatura, Tempo, Início, Vencimento, Renovação, auto ON default em novas + flip em massa onde `billing_end IS NULL`, Fim só com auto OFF e projeção como ajuda), reajuste condicional à regra + aniversário default `billing_start + 1 ano` (backfill 27 séries), seção `Valores da recorrência` com alerta persistente, **bloqueio** de série sem períodos/faixas, baixa em lote no `PaymentToggle` (esconde não-cobrar/suspensas). Migrations `20260917090000`, `20260918090000`, `20260918090001`, `20260918090002`.
  ...
  └── Table (accordion lazy, 1 RPC per first expand)
      └── Row (expandable)
          ...
          └── Expanded (lazy, bg-bg-secondary/20):
              ├── Barra: ViewToggle + badges (Exceção · Suspenso até dd/mm · Reajuste 4,62% em 03/2026) + PaymentToggle + CSV/PDF buttons + "+ Exceção"
              ...
              ├── Cobranças eventuais do mês (linha própria, soma no total, fora do MRR)
              ├── Exceções vigentes (escopo Cliente/Série, tipo, vigência, motivo, created_by/at) — zeradas (suspenso/não cobrar/isento/pausado) não aparecem no cockpit desde 2026-09-18 (revoga Q4)
...
**State management:** TanStack Query (`staleTime` cockpit `5min`, months `10min`), `openSet: Set<clientId>`, `detailCache: { [clientId]: {loading, error, data} }`, `exportView: 'geral'|'faturavel'` (`isento` removida 2026-09-18 — zerados não aparecem), `csvDropdownOpen`, `exceptionDrawer`, `paymentToggling`.
...
- [x] **CSV:** `EXPORT_VIEWS = { geral, faturavel }` (`isento` removida 2026-09-18 — zerados não aparecem; `faturavel` = `mrr_real > 0` ou `eventuais > 0`); `csvSintetico(rows)` com `Cliente | CNPJ | SaaS_ID | Tipo | Piso | Uso | Billable | Valor unit. | MRR mínimo | MRR real | Excedente | Eventuais | Exceção | Escopo | Adimplência | Δ`; `csvAnaliticoGlobal` via `supabase.rpc('get_financeiro_export')` (filtra pelos ids visíveis) + `csvAnaliticoRow` via `get_financeiro_detalhe` (séries + módulos + exceções + profissionais concatenados)
...
| 1.4 | 2026-09-16 | DoncCX Hub | Adendo papéis de escrita: `billing_payments`/`billing_exceptions`/`contract_series` write = `admin,manager,finance,sales` (migrations `20260916120000` + `20260916120001` REVOKE EXECUTE no trigger); trigger `sync_billing_payments_delay_days` virou `SECURITY DEFINER`; RLS habilitado em `client_handover_templates` (SELECT mesmos papéis de leitura); fim "Automático: N meses" do form não é mais persistido (`NULL` = em aberto); botão "Lançar" alinhado à flag `financeiro_cockpit_write`; toasts de permissão PT-BR sem duplicar |
