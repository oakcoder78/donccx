...
- **MRR obrigatória + redesign do Contrato (2026-09-17/18, pré-produção):** fonte de verdade = **só o lançado** (períodos/faixas/eventuais); base do plano é referência visual e nunca fatura. `_financeiro_series_month`: contratado = `rules_total` do mês (sem fallback base); `uso_os` = OS criadas (`snapshot.totalOs ?? os_created`, soma instâncias); exclusão olha o contratado (`raw_min==0` sem pausa/zerada/isenção/regra-no-mês/exceção-na-série) — excedente sozinho não fatura; regra 0 no mês aparece zerada. `get_financeiro_cockpit`: Uso/Billable `por_os` em OS criadas + coluna `eventuais`; `get_financeiro_detalhe`: payload `eventuais` (linha própria, soma no total do mês). Form: cabeçalho reagrupado (Assinatura, Tempo, Início, Vencimento, Renovação, auto ON default em novas + flip em massa onde `billing_end IS NULL`, Fim só com auto OFF e projeção como ajuda), reajuste condicional à regra + aniversário default `billing_start + 1 ano` (backfill 27 séries), seção `Valores da recorrência` com alerta persistente, **bloqueio** de série sem períodos/faixas, baixa em lote no `PaymentToggle`. Migrations `20260917090000`, `20260918090000`, `20260918090001`.
...
| 1.6 | 2026-09-18 | DoncCX Hub | Fonte de verdade = só o lançado: motor rules-only (contratado = `rules_total`, sem fallback base; `uso_os` = OS criadas; exclusão pelo contratado, regra 0 aparece zerada); `eventuais` em linha própria (cockpit + detalhe + sintético); flip `auto_renew` em massa (só 29 mantém fim); validação invertida (base sozinha não passa) + alerta persistente |
