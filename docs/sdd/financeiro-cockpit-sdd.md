...
- **MRR obrigatória + redesign do Contrato (2026-09-17, pré-produção):** `_financeiro_series_month` exclui série sem valor faturado (`raw_min==0 AND raw_real==0` sem pausa/zerada/isenção) — sem MRR lançada, sem lançamento; pausadas/zeradas/isentas seguem visíveis zeradas. Form: cabeçalho reagrupado (Assinatura, Tempo, Início, Vencimento, Renovação, auto ON default em novas, Fim só com auto OFF e projeção como ajuda), reajuste condicional à regra + aniversário default `billing_start + 1 ano` (backfill 27 séries), seção renomeada `Valores da recorrência`, **bloqueio** de série ativa faturável sem valor contratado, baixa em lote no `PaymentToggle` (`adimplente` + pago no vencimento). Migration `20260917090000`.
