...
- **Papéis revisados (revoga Q4a):** o cockpit é restrito a `admin`, `manager` e `finance` (flag `cockpit_financeiro`; `financial_data`, hoje com `sales` também no `allowed_roles`, não muda isso — o cockpit checa só `cockpit_financeiro` desde 2026-09-13). Exceções/correções/adimplência: escrita `admin/finance` (flag `financeiro_cockpit_write`); `manager` leitura+export; `sales/csm/analyst` sem acesso ao cockpit. Vendas continua negociando e editando séries/contrato na carteira (Empresas) — e, desde 2026-09-13, também vê o valor financeiro pelo formulário de edição (`empresas_mutate`), não mais pelo `SAFE_CLIENT_COLS` (removido, ver `docs/modules/clients.md`).
